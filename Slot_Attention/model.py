...
class SlotAttentionAutoEncoder(layers.Layer):
  ...
  def __init__(self, resolution, num_slots, num_iterations):
    ...
    if resolution[0] == 64:
      ...
      self.kernel_size = 5
    elif resolution[0] == 128:
      self.encoder_channel = 64
      self.slot_size = 64
      self.mlp_hidden_size = 128
      self.kernel_size = 5
    elif resolution[0] == 256:
      self.encoder_channel = 128
      self.slot_size = 128
    ...
    self.encoder_cnn = tf.keras.Sequential([
        layers.Conv2D(self.encoder_channel, kernel_size=self.kernel_size, padding="SAME", activation="relu"),
        layers.Conv2D(self.encoder_channel, kernel_size=self.kernel_size, padding="SAME", activation="relu"),
        layers.Conv2D(self.encoder_channel, kernel_size=self.kernel_size, padding="SAME", activation="relu"),
        layers.Conv2D(self.encoder_channel, kernel_size=self.kernel_size, padding="SAME", activation="relu")
    ], name="encoder_cnn")

    # self.decoder_initial_size = (tf.cast(resolution[0]/16, dtype=tf.int32), tf.cast(resolution[1]/16, dtype=tf.int32))
    ...
    if resolution[0] == 64:
      ...
      # self.decoder_initial_size = (32, 32)
      # self.decoder_initial_size = (16, 16)
      self.decoder_cnn = tf.keras.Sequential([
          layers.Conv2DTranspose(
              128, self.kernel_size, strides=(2, 2), padding="SAME", activation="relu"),
          layers.Conv2DTranspose(
              128, self.kernel_size, strides=(2, 2), padding="SAME", activation="relu"),
          layers.Conv2DTranspose(
              128, self.kernel_size, strides=(2, 2), padding="SAME", activation="relu"),
          ...
          layers.Conv2DTranspose(
              128, self.kernel_size, strides=(2, 2), padding="SAME", activation="relu"),
          layers.Conv2DTranspose(
              128, self.kernel_size, strides=(1, 1), padding="SAME", activation="relu"),
          layers.Conv2DTranspose(
              4, 3, strides=(1, 1), padding="SAME", activation=None)
      ], name="decoder_cnn")
    ...
    self.encoder_pos = SoftPositionEmbed(self.encoder_channel, self.resolution)
    self.decoder_pos = SoftPositionEmbed(self.slot_size, self.decoder_initial_size)
    self.encoder_dense = layers.Dense(self.encoder_channel, use_bias=True)
    self.decoder_dense = layers.Dense(self.slot_size, use_bias=True)

    self.layer_norm = layers.LayerNormalization()
    self.mlp = tf.keras.Sequential([
        layers.Dense(self.slot_size, activation="relu"),
        layers.Dense(self.slot_size)
    ], name="feedforward")
  ...
  def call(self, image):
    ...
    '''initial (4, 256, 256, 3)
    after encoder_cnn (4, 256, 256, 128)
    after encoder_pos (4, 256, 256, 128)
    after spatial_flatten (4, 65536, 128)
    after mlp (4, 65536, 64)
    slot (4, 6, 64)
    after spatial_broadcast (24, 8, 8, 64)
    after decoder_pos (24, 8, 8, 64)
    after decoder_cnn (24, 256, 256, 4)'''
    ...
    out_features = x
    # ## TODO: ablation with traditional clustering algorithems
    # print(x.shape, type(x))
    # for idx in range(0, batch_size):
    #   numpy_x = x.eval(session=tf.compat.v1.Session())
    #   kmeans = KMeans(n_clusters=self.num_slots, random_state=0).fit(numpy_x[idx])
    ...
    # return recon_combined, recons, masks, slots, out_features

    return recon_combined, recons, masks, slots
  
  def get_summary(self, image):
    stringlist = []
    tf.keras.Model(inputs=image, outputs=self.call(image), name="SlotAttentionAutoEncoder").summary(print_fn=lambda x: stringlist.append(x))
    short_model_summary = "\n".join(stringlist)
...
def build_model(resolution, batch_size, num_slots, num_iterations,
  ...
  # summary = model_def(resolution, num_slots, num_iterations).get_summary(image)
  return model
