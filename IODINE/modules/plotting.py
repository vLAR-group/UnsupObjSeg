...
import numpy as np
from modules.mask_to_boundary import K_mask_to_boundary, mask_to_boundary
...
@optional_clean_ax
## m: (K, H, W)
def show_mask(m, ax, mask=None):
  assert len(m.shape) == 3
  color_mask = np.argmax(m, axis=0)
  color_mask += 1 ## not use black color
  if mask is not None:
    color_mask = color_mask * mask ## black color is reserved for unmasked area
  color_mask = color_mask.astype(float) / m.shape[0]
  return ax.imshow(color_mask.clip(0.0, 1.0), interpolation="nearest", vmin=0)

@optional_clean_ax
## m: (K, H, W)
def show_boundary(m, ax, mask=None):
  assert len(m.shape) == 3
  color_mask = np.argmax(m, axis=0)
  color_mask += 1 ## not use black color
  if mask is not None:
    color_mask = color_mask * mask ## black color is reserved for unmasked area
  color_mask = K_mask_to_boundary(color_mask)
  color_mask = color_mask.astype(float) / m.shape[0]
  return ax.imshow(color_mask.clip(0.0, 1.0), interpolation="nearest", vmin=0)

@optional_clean_ax
def show_binary_mask(m, ax):
  assert len(m.shape) == 2
  assert np.min(m) >= 0 
  assert np.max(m) <= 1 
  color_mask = m
  color_mask = color_mask.astype(float)
  return ax.imshow(1-color_mask.clip(0.0, 1.0), interpolation="nearest", cmap='binary')

def show_binary_boundary(m, ax):
  assert len(m.shape) == 2
  assert np.min(m) >= 0 
  assert np.max(m) <= 1 
  color_mask = mask_to_boundary(m.astype(np.uint8))
  color_mask = color_mask.astype(float)
